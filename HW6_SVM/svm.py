...
class SVM(object):
    ...
    def predict(self, X):
        ...
        for sample in X:
            pred = 0
            for i in range(len(self.alpha)):
                pred += self.alpha[i] * self.sv_label[i] * self.kernel(self.sv[i], sample)
            pred += self.intercept
    ...
    def output(self, X, y):
        ...
        plt.scatter(X[:, 0], X[:, 1], c=y, cmap='bwr', alpha=1, s=10)
...
if __name__ == "__main__":
    ...
    plt.scatter(X[:, 0], X[:, 1], c=y, cmap='bwr', alpha=1, s=20)
